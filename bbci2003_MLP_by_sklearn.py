...
# module in this repo
from np_train_data_100Hz import read_train_data, read_test_data
...
data_x, data_y = read_train_data(SOURCE_ROOT_DIR)
...
def do_output(model_dir, input_dir, output_dir):
    ...
    test_data_x = read_test_data(input_dir)
...
if __name__ == "__main__":
    ...
    # save nn model
    pkl_dir = os.path.join(SOURCE_ROOT_DIR, "SKPickle", "sk_model.pkl")
    with open(pkl_dir, "wb") as f:
        pickle.dump(nn, f)

    # use nn model
    input_dir = os.path.join(SOURCE_ROOT_DIR, "inputData", "sp1s_aa_test.csv")
